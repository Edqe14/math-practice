...
const getColorByTime = (time: number): [string, string, string] => {
  if (time <= 60 * 1000) return ['text-green-600', 'bg-green-100', 'border-green-600']; // 1 min
  if (time <= 60 * 2 * 1000) return ['text-yellow-600', 'bg-yellow-100', 'border-yellow-600']; // 2 min
    ...
    <nav className="bg-red-500 text-white py-4 px-8 flex justify-between items-center">
      ...
      <section>
        <span className="cursor-pointer transition-colors z-50 flex gap-3">
          <ArrowCounterClockwise weight="bold" size={24} onClick={() => {
            all.clear();
            resetTimer();
          }} />
        ...
        <Drawer size="xl" position="right" opened={openSide} onClose={() => setOpenSide(false)} title="Questions" classNames={{ title: 'text-2xl font-semibold', drawer: 'overflow-y-auto text-zinc-700', header: 'p-8 sticky bg-white top-0 mb-0 z-10' }}>
              ...
              return (
                ...
                >
                  ...
                  <section className="flex items-center">
                    {!all.has(i + 1) && <p className="text-sm px-1 border rounded border-zinc-700 bg-zinc-200 opacity-50">Not attempted</p>}

                    {all.has(i + 1) && (
                      <span className={`text-sm px-1 border rounded ${text} ${bg} ${border}`}>
